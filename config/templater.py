...
def run_command(matched_script, current_platform):
    ...
    try:
        if script_type == "python":
            os.system(f"python3 {script_path}" if current_platform != "windows" else f"python {script_path}")
        elif script_type == "batch":
            os.system(f"{script_path}" if current_platform == "windows" else f"echo 'Batch file can only run on Windows'")
        elif script_type == "bash":
            os.system(f"bash {script_path}" if current_platform == "linux" else f"echo 'Bash script can only run on Linux'")
        else:
...
def main():
    ...
    while True:
        ...
        if user_input == "q":
            ...
            matched_script = next(
                (script for script in configs["scripts"] if script["command"] == user_input),
                None,
            )
